...
<body>
		...
		<table border="0" style="width: 100%">
			...
			<tr>
				...
				<td><input id="name" name="name" class="mini-textbox"
					emptyText="请输入部门名称" required="true" onvalidation="onValidation"
					errorMode="none" vtype="remote" requiredErrorText="部门名称不能为空"
	...
	<script type="text/javascript">
		...
		function init(departmentId) {
			$
					.ajax({
						url : '<%=basePath%>department/getDepartmentById.json',
						type : 'get',
						data : {
		...
		function onEdit(e) {
			...
			$
					.ajax({
						url : '<%=basePath%>department/editDepartment.json',
						type : 'post',
